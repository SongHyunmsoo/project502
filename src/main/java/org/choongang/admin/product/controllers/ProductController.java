...
@Controller("adminProductController") // 중복 에러를 위한 코드
...
public class ProductController implements ExceptionProcessor {
     ...
     */
    @GetMapping
    public String list(Model model) {   // 상품목록
    ...
    private void commonProcess(String mode, Model model) {
        ...
        List<String> addScript = new ArrayList<>();     // 관리자용 스크립트

        if (mode.equals("add") || mode.equals("edit")) {
            pageTitle = mode.equals("edit") ? "상품 수정" : "상품 등록";
            addCommonScript.add("ckeditor5/ckeditor");
            addCommonScript.add("fileManager");
            addScript.add("product/form"); // 상품 분류 양식
